...
class GymWrapper(Wrapper):
    ...
    def labeled_observations(self, space, obs, sbs=""):
        if isinstance(space, sp.Tuple):
            ...
            return convert_percept(label, *obs)
            ...
            for k in obs.keys():
                ...
                if isinstance(space[k], sp.Discrete):
                    obs_list+=convert_percept(label, obs[k])
                    ...
                    obs_list+=convert_percept(label, *l)
    ...
    def parse_world_state(self, ospace, obs, reward, done):
        ...
        return convert_percept("Reward", reward)[0], obs_list, done
