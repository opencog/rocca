...
def Menger(xorg, yorg, zorg, size, blocktype, variant, holetype):
    # draw solid chunk
    genstring = GenCuboidWithVariant(xorg, yorg, zorg, xorg + size - 1, yorg + size - 1, zorg + size - 1, blocktype,
                                     variant) + "\n"
    # now remove holes
    unit = size
    while (unit >= 3):
...
def GenCuboidWithVariant(x1, y1, z1, x2, y2, z2, blocktype, variant):
    return '<DrawCuboid x1="' + str(x1) + '" y1="' + str(y1) + '" z1="' + str(z1) + '" x2="' + str(x2) + '" y2="' + str(
        y2) + '" z2="' + str(z2) + '" type="' + blocktype + '" variant="' + variant + '"/>'
...
missionXML = '''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
            ...
            <ServerSection>
              ...
              <ServerHandlers>
                  ...
                  <DrawingDecorator>
                    <DrawSphere x="-27" y="70" z="0" radius="30" type="air"/>''' + Menger(-40, 40, -13, 27, "stone",
                                                                                          "smooth_granite", "air") + '''
...
if __name__ == '__main__':
    ...
    while not done:
        print(".", end="")
        time.sleep(0.2)
        rw, ob, done = malmoWrapper.step(mk_action("jump", 1),
